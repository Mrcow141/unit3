...
void mouseReleased() {
  ...
  //save button
  if (mouseX>40&&mouseX<160&&mouseY>800&&mouseY<850) {
    selectOutput("choose a name for your file", "saveimage");
  }
  //load button
  if (mouseX>200&&mouseX<320&&mouseY>800&&mouseY<850) {
    selectInput("pick and image to load", "openImage");
  }
}
...
void mouseDragged() {
  if (mouseX > 400) {
    if (yorudOn == false) {
      ...
      image(yorud, mouseX, mouseY, stampTool, stampTool);
    }
  }
  controlSlider();
}
...
void controlSlider() {
  ...
  thickness = map(sliderX,40,360,1,8);
  stampTool = map(sliderX,40,360,100,400);
}
...
void saveimage(File f) {
  if (f!=null) {
    PImage canvas = get(401, 0, 600, 600);
